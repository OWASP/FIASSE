...
Treating the security controls you have chosen out of a control-catalog as the singular mechanism by which software is secured is a fallacy. Control catalogs (like NIST 800-53, ISO 27001 Annex A, PCI DSS, and their equivalents) support risk management, assurance, and external evaluation. They articulate what protections must exist within a system. However, they do not implicitly construct those protections. Software does not become defensible simply because a security control exists. Those controls must be used as input that informs security requirements. This is results in development implementing the features. Verification is then done by examining the features that fulfill corresponding requirements. This results in harmony between security and development.

The fallacy results in security shortcutting the process or skipping it all together. This causes friction between security and engineering teams because security is insinuating non-existing requirements on development. When controls are used as context for requirements, implementations are planed and integrated into architecture layers where needed. This increases developments ability to ensure complete implementation and longevity of that implementation.

The forcing implicit implementation produces systems that pass audits but fail under sustained attack, because audits merely measure the existence of the control. It also produces development teams who see security as hostile demands rather than a property of their work.

The corrective discipline is the first FIASSE value (Section 2). Engineering attention belongs on qualities that make features complete and software defensible. Security control catalogs a tool for security to participate in requirements and provide assurance and risk functions that inform the code creation process. Mistaking controls for implemented security features is a fallacy.
